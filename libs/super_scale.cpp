...
int SuperScale_::init(const std::string &proto_path, const std::string &model_path) {
    srnet_ = dnn::readNetFromCaffe(proto_path, model_path);
    net_loaded_ = true;
    return 0;
}

Mat SuperScale_::processImageScale(const Mat &src, float scale, const bool &use_sr,
                                  int sr_max_size) {
    Mat dst = src;
    if (scale == 1.0) {  // src
        return dst;
...
int SuperScale_::superResoutionScale(const Mat &src, Mat &dst) {
    Mat blob;
